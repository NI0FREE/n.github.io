var storyList =  [{status:0,summ:2311,senderID:1,time:"1320"},
                  ...
                  {status:1,summ:1110000,senderID:1,time:"0322"},
                  {status:1,summ:45354,senderID:1,time:"0322"},
                  {status:1,summ:500,senderID:1,time:"0322"},
                  {status:1,summ:122,senderID:1,time:"0322"},
                  {status:0,summ:6000,senderID:1,time:"0322"},
                  {status:0,summ:122500,senderID:1,time:"0322"},
                  {status:1,summ:10000000,senderID:1,time:"0322"},
                  {status:0,summ:12785,senderID:1,time:"0322"},
                  {status:1,summ:2235,senderID:1,time:"0322"},
                  {status:1,summ:2235,senderID:1,time:"0322"},
...
function storyUpdate(){
    ...
    for(let i = 0; i < storyList.length; i++){
        ...
        listpayersIDS2.push(rint(1000000000,9000000000));
        var storyContaner;
        
        if(storyList[i].status){
            storyContaner = 
             `
             <div class="infoCard"  id="`+listpayersIDS2[i]+`"  onclick="`+'showNoty('+' `Время операции:  '+storyList[i].time[0]+storyList[i].time[1]+':'+storyList[i].time[2]+storyList[i].time[3]+'(МСК) <br> Номер карты отправителя: <br>'+cardSEND+' <br> Номер карты получателя: <br>'+cardUPPR+'<br> Сумма операции: '+storyList[i].summ+'f`,2)'+`" style="opacity:0; background: #7ba97a;height: 52px;position: relative;overflow: hidden;">
            ...
            storyContaner = 
            `
            <div class="infoCard"  id="`+listpayersIDS2[i]+`"  onclick="`+'showNoty('+' `Время операции:  '+storyList[i].time[0]+storyList[i].time[1]+':'+storyList[i].time[2]+storyList[i].time[3]+'(МСК) <br> Номер карты отправителя: <br>'+cardSEND+' <br> Номер карты получателя: <br>'+cardUPPR+'<br> Сумма операции: '+storyList[i].summ+'f`,2)'+`"  style="opacity:0; background: #7a91a9;height: 52px;position: relative;overflow: hidden;">
    ...
    var time= 100;
    for(let i = 0;i< listpayersIDS2.length; i ++){
        let $fix = document.getElementById(listpayersIDS2[i]);
        
        setTimeout(()=>{
           $fix.classList.add('notyAnim-33');
           $fix.style.opacity = 1;
        },time);
        time+=25;
        //centerFix3(listpayersIDS2[i]);
    }
...
// function centerFix3(id){
//     let $fix = document.getElementById(id);
    
//     setTimeout(()=>{
//             $fix.classList.remove('notyAnim-2-2');
//             $fix.classList.add('notyAnim-2');
//            // $fix.style.opacity = 1;
//     },100);
// }
