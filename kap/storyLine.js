var storyList =  [{status:0,summ:2311,senderID:1,time:"1320"},
                  ...
                  {status:0,summ:1110000,senderID:1,time:"322"},
                 ];
...
function storyUpdate(){
    ...
    for(let i = 0; i < storyList.length; i++){
        ...
        if(storyList[i].status){
            storyContaner = 
             `
             <div class="infoCard" onclick="`+'showNoty('+' `Время операции:  '+storyList[i].time[0]+storyList[i].time[1]+':'+storyList[i].time[2]+storyList[i].time[3]+'(МСК) <br> Номер карты отправителя: '+cardSEND+' <br> Номер карты получателя: '+cardUPPR+'<br> Сумма операции: '+storyList[i].summ+'f`,2)'+`" style="background: #7ba97a;height: 52px;position: relative;overflow: hidden;">
             <div class="userinf standartTitle" style="position: relative;z-index: 10;margin-top: 14.5px;width: calc(50% - 41px);" id="nameMine">Входящий платеж</div>
            ...
            <div class="userinf standartTitle" style="position: relative;z-index: 10;width: calc(50% - 41px);margin-top: 14.5px;" id="nameMine">Исходящий платеж</div>
